...
|`helm-charts/` | Contains the Helm charts for AMD GPU Operator, can be generated by ```make helm```. Each time of generating the whole ```helm-charts/``` folder will be deleted and re-created again, the customized change of helm charts should be put into ```hack/``` folder and ```make helm``` command will apply the customize-changed files from ```hack/``` folder to ```helm-charts/``` folder
